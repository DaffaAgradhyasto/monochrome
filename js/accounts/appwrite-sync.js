...
const syncManager = {
                ...
                try {
                    const newRecord = await databases.createDocument(
                        ...
                        ID.unique(),
                        {
                            ...
                            favorite_albums: JSON.stringify([]),
    ...
    async getUserData() {
        ...
        const userSettings = this.safeParseInternal(record.user_settings, 'user_settings', {});
        return { library, history, userPlaylists, userFolders, profile, userSettings };
    },
    ...
    async syncSettings(settings) {
        ...
        try {
            ...
            console.log('[Appwrite] Settings synced successfully');
        } catch (err) {
            console.error('[Appwrite] Failed to sync settings:', err);
        }
    },
    ...
    async updateProfile(data) {
        ...
        if (updateData.privacy && typeof updateData.privacy !== 'string') {
            updateData.privacy = JSON.stringify(updateData.privacy);
        }
        if (updateData.favorite_albums && typeof updateData.favorite_albums !== 'string') {
            updateData.favorite_albums = JSON.stringify(updateData.favorite_albums);
        }
        ...
        if (user) {
            ...
            try {
                ...
                if (cloudData) {
                    ...
                    if (cloudData.userSettings && typeof cloudData.userSettings === 'object') {
                        Object.entries(cloudData.userSettings).forEach(([key, value]) => {
                            try {
                                localStorage.setItem(key, typeof value === 'string' ? value : JSON.stringify(value));
                            } catch (e) {
                                console.error('[Appwrite] Failed to restore setting:', key, e);
                            }
                        });
                    }
