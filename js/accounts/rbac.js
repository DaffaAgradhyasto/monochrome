...
export const rbacManager = {
  ...
  async promoteToAdmin(targetUserId) {
    if (!this.isOwner()) throw new Error('Only Owner can promote users to Admin.');
  ...
  async demoteFromAdmin(targetUserId) {
    if (!this.isOwner()) throw new Error('Only Owner can demote Admins.');
    const result = await databases.listDocuments(DATABASE_ID, USERS_COLLECTION, [
      Query.equal('user_id', targetUserId),
      Query.limit(1),
    ]);
  ...
  async setBanStatus(targetUserId, isBanned) {
    if (!this.isAdmin()) throw new Error('Only Admin or Owner can ban users.');
    const result = await databases.listDocuments(DATABASE_ID, USERS_COLLECTION, [
      Query.equal('user_id', targetUserId),
      Query.limit(1),
    ]);
  ...
  async softDeletePlaylist(playlistId) {
    if (!this.isAdmin()) throw new Error('Only Admin or Owner can delete playlists.');
    ...
    if (result.documents.length > 0) {
      await databases.updateDocument(DATABASE_ID, PUBLIC_COLLECTION, result.documents[0].$id, {
        is_deleted: true,
  ...
  async restorePlaylist(playlistId) {
    if (!this.isAdmin()) throw new Error('Only Admin or Owner can restore playlists.');
    const PUBLIC_COLLECTION = 'public_playlistspublic_playlists';
    const result = await databases.listDocuments(DATABASE_ID, PUBLIC_COLLECTION, [
      Query.equal('uuid', playlistId),
      Query.limit(1),
  ...
  async getAllUsers(page = 1, perPage = 25, search = '') {
    if (!this.isAdmin()) throw new Error('Only Admin or Owner can view all users.');
    const queries = [Query.limit(perPage), Query.offset((page - 1) * perPage), Query.orderDesc('$createdAt')];
    if (search) queries.push(Query.search('username', search));
    const result = await databases.listDocuments(DATABASE_ID, USERS_COLLECTION, queries);
    return result;
  },

  /** Get audit logs (Admin+) */
  async getAuditLogs(page = 1, perPage = 50) {
    if (!this.isAdmin()) throw new Error('Only Admin or Owner can view audit logs.');
    try {
      const result = await databases.listDocuments(DATABASE_ID, AUDIT_COLLECTION, [
        Query.limit(perPage),
  ...
  async logAudit(action, meta = {}) {
    ...
    try {
      await databases.createDocument(DATABASE_ID, AUDIT_COLLECTION, ID.unique(), payload);
    } catch (e) {
  ...
  _updateRoleBadge(role) {
    ...
    const avatarBtn = document.querySelector('#profile-avatar-btn, .avatar-btn, #nav-profile');
    if (!avatarBtn) return;
