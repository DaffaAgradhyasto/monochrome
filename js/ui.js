...
export class UIRenderer {
    ...
    async renderTrackPage(trackId, provider = null) {
      ...
      async renderHotNewPage() { this.showPage('hot-new'); const container = document.getElementById('hot-new-content'); if (!container) return; container.innerHTML = this.createSkeletonCards(12); try { const response = await fetch('https://hot.monochrome.tf/'); if (!response.ok) throw new Error('Failed to load Hot & New data'); const data = await response.json(); container.innerHTML = ''; const GENRES = [ { id: 'hip_hop', name: 'Hip-Hop' }, { id: 'rnb', name: 'R&B / Soul' }, { id: 'blues', name: 'Blues' }, { id: 'classical', name: 'Classical' }, { id: 'country', name: 'Country' }, { id: 'dance_electronic', name: 'Dance & Electronic' }, { id: 'americana', name: 'Folk / Americana' }, { id: 'world', name: 'Global' }, { id: 'gospel', name: 'Gospel / Christian' }, { id: 'jazz', name: 'Jazz' }, { id: 'kpop', name: 'K-Pop' }, { id: 'kids', name: 'Kids' }, { id: 'latin', name: 'Latin' }, { id: 'metal', name: 'Metal' }, { id: 'pop', name: 'Pop' }, { id: 'reggae', name: 'Reggae / Dancehall' }, { id: 'retro', name: 'Legacy' }, { id: 'indierock', name: 'Rock / Indie' }, ]; const genresSection = document.createElement('section'); genresSection.className = 'content-section'; genresSection.innerHTML = '<h2 class="section-title">Genres</h2>'; const genresGrid = document.createElement('div'); genresGrid.style.cssText = 'display:flex;flex-wrap:wrap;gap:0.5rem;'; genresGrid.innerHTML = GENRES.map(genre => `<button class="genre-card" data-genre-id="${genre.id}" data-genre-name="${escapeHtml(genre.name)}" style="padding:0.5rem 1rem;border-radius:var(--radius);background:var(--card);border:1px solid var(--border);cursor:pointer;font-size:0.9rem;">${escapeHtml(genre.name)}</button>`).join(''); genresSection.appendChild(genresGrid); container.appendChild(genresSection); genresGrid.querySelectorAll('.genre-card').forEach(card => { card.addEventListener('click', () => { this.renderGenrePage(card.dataset.genreId, card.dataset.genreName); }); }); if (data.top_albums && data.top_albums.length > 0) { this.renderExploreSection(container, 'Trending Albums', data.top_albums, 'album'); } if (data.top_tracks && data.top_tracks.length > 0) { this.renderExploreSection(container, 'Trending Tracks', data.top_tracks, 'track'); } if (data.featured_playlists && data.featured_playlists.length > 0) { this.renderExploreSection(container, 'Featured Playlists', data.featured_playlists, 'playlist'); } if (data.sections && data.sections.length > 0) { data.sections.forEach(section => { if (section.items && section.items.length > 0) { let type = null; if (section.type === 'ALBUM_LIST') type = 'album'; else if (section.type === 'TRACK_LIST') type = 'track'; else if (section.type === 'PLAYLIST_LIST') type = 'playlist'; if (type) { this.renderExploreSection(container, section.title, section.items, type); } } }); } if (container.children.length === 0) { container.innerHTML = createPlaceholder('No Hot & New content available.'); } } catch (e) { console.error(e); container.innerHTML = createPlaceholder('Failed to load Hot & New content.'); } } async renderMoodPage() {
    this.showPage('mood');
