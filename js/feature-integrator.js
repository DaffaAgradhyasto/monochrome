...
export async function initializeNewFeatures(deps) {
    ...
    try {
        ...
        audioPlayer.addEventListener('timeupdate', () => {
            if (player.currentTrack && audioPlayer.duration) {
                heatmap.record(player.currentTrack.id, audioPlayer.currentTime, audioPlayer.duration);
            }
        });
    ...
    // 27. Year in Review
    try {
        const { YearInReview } = await import('./year-in-review.js');
        const yearInReview = new YearInReview();
        window.monochromeYearInReview = yearInReview;
        console.log('[Features] Year in Review initialized');
    } catch (e) {
        console.warn('[Features] Failed to init Year in Review:', e);
    }
